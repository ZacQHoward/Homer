...
//----------PORT NOTES----------
//This trimmed version is set up for a PlatformIO port on a XIAO ESP32C3.
//The first pass keeps the melty control loop and disables AVR-only features.

//----------DIAGNOSTICS----------
//#define JUST_DO_DIAGNOSTIC_LOOP                 //Disables the robot / just displays config / battery voltage / RC info via serial

//----------EEPROM----------
//EEPROM config is disabled for the first port.
//If you want persistent tuning later, add a storage layer for the ESP32.
// #define ENABLE_EEPROM_STORAGE
// #define EEPROM_WRITTEN_SENTINEL_VALUE 42

//----------TRANSLATIONAL DRIFT SETTINGS----------
//"DEFAULT" values are used directly in this first port.
//If you add persistent storage later, these become fallback values.
#define DEFAULT_ACCEL_MOUNT_RADIUS_CM 5.3         //Radius of accelerometer from center of robot
...
#define DEFAULT_ACCEL_ZERO_G_OFFSET 2.75f        //Value accelerometer returns with robot at rest (in G) - adjusts for any offset
...
//----------SAFETY----------
//Watchdog is disabled for now while the port is being stabilized.
