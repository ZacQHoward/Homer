...
static const uint16_t BASE_SPIN_MAX_OFFSET_US = (uint16_t)(HomerConfig::BASE_SPIN_MAX_OFFSET_PERCENTAGE * 500.0f) - ESC_NEUTRAL_US;

static const uint16_t RC_MAX_OFFSET_US = (RcConfig::RC_MAX_US - RcConfig::RC_MIN_US) / 2;
...
static const float MAX_TRANSLATION_ROTATION_INTERVAL_US =
    (1.0f / HomerConfig::MIN_TRANSLATION_RPM) * 60.0f * 1000.0f * 1000.0f;
...
static float accel_mount_radius_cm = HomerConfig::ACCEL_MOUNT_RADIUS_CM;
static float accel_zero_g_offset = HomerConfig::DEFAULT_ACCEL_ZERO_G_OFFSET;
...
static int get_centered_channel_us(uint16_t pulse_width_us) {
  int centered = (int)pulse_width_us - (int)RcConfig::RC_NEUTRAL_US;
...
static float get_spin_direction(const RcInput& input) {
  int throttle_centered = get_centered_channel_us(input.ch3_us);
  if (throttle_centered > (int)THROTTLE_DEADBAND_US) return -1.0f;
  if (throttle_centered < -((int)THROTTLE_DEADBAND_US)) return 1.0f;
  return 0.0f;
}

static float get_spin_magnitude_fraction(const RcInput& input) {
  int throttle_centered = get_centered_channel_us(input.ch3_us);
...
static translation_vector_t compute_translation_vector(const RcInput& input) {
  int centered_x = apply_deadband(get_centered_channel_us(input.ch1_us), TRANSLATION_DEADBAND_US);
  int centered_y = apply_deadband(get_centered_channel_us(input.ch2_us), TRANSLATION_DEADBAND_US);
...
static void update_manual_phase_trim(const RcInput& input, float rotation_interval_us) {
  int trim_centered = apply_deadband(get_centered_channel_us(input.ch4_us), TRIM_DEADBAND_US);
  if (trim_centered == 0) {
    return;
  }
...
void spin_one_rotation(const RcInput& input) {
  ...
  translation_vector_t translation_vector = compute_translation_vector(input);
  float rotation_interval_us = compute_rotation_interval_us();

  update_manual_phase_trim(input, rotation_interval_us);
