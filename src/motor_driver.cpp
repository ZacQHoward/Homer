...
static uint16_t invert_pulse_us(uint16_t pulse_width_us) {
    pulse_width_us = clamp_pulse_us(pulse_width_us);
    return static_cast<uint16_t>(MotorConfig::ESC_NEUTRAL_US * 2U - pulse_width_us);
}
...
void motor_1_write_us(uint16_t pulse_width_us) {
    motor_write_us(MotorConfig::MOTOR_1_PWM_CHANNEL, invert_pulse_us(pulse_width_us));
}
